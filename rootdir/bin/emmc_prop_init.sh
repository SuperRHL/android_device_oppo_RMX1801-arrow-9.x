...
setprop oppo.storage.primary.status $emmc_health

emmc_size=`cat /sys/bus/mmc/devices/mmc0\:0001/emmc_total_size`
setprop oppo.storage.primary.size "$emmc_size"G

emmc_preEOL=${emmc_health:0:4}
setprop oppo.storage.primary.health $emmc_preEOL

setprop oppo.storage.primary.type 'eMMC'

emmc_healthtypeA=`cat /sys/bus/mmc/devices/mmc0\:0001/emmc_health_A`
setprop oppo.storage.primary.healthtypeA $emmc_healthtypeA

emmc_healthtypeB=`cat /sys/bus/mmc/devices/mmc0\:0001/emmc_health_B`
setprop oppo.storage.primary.healthtypeB $emmc_healthtypeB
